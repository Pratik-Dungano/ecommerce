...
const AddLookForm = ({ onClose }) => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.data.products) {
          ...
          const productsWithVideos = response.data.products.filter(
        ...
        toast.error('Failed to fetch products with videos');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddProduct = () => {
    ...
    const selectedProductData = availableProducts.find(p => p._id === selectedProduct);
    ...
    setFormData({
      ...
      products: [
        ...formData.products,
        {
          ...
          // Add these just for display purposes in the UI
          _tempName: selectedProductData.name,
          _tempImage: selectedProductData.image[0],
          _tempVideo: selectedProductData.video[0]
        }
      ]
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to create look');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 3, gap: 2 }}>
        ...
        >
          {submitting ? <CircularProgress size={24} /> : 'Create Look'}
        </Button>
      </Box>
    </Box>
  );
